...
function Home() {
  ...
  const fetchResearchArticlesCount = async (authorId) => {
    try { 
      const response = await axios.get(`${host}articles/getByArthorId/${authorId}`)
      const data = response.data.length;
      return data // Assuming the API response contains the count
    } catch (error) {
      ...
      try {
        ...
        const updatedData = await Promise.all(
          response.data.map(async (item) => {
            const count = await fetchResearchArticlesCount(item._id);
            console.log("count = ",count)
            return { ...item, researchArticlesCount: count };
          })
        );
  ...
  return (
    <Container maxWidth="xl" sx={{ paddingTop: "3rem" }} style={{ marginTop: "65px" }}>
      {isLoading ? (
        <div
          style={{
            display: "flex",
                      ...
                      <div class="card">
                        <div class="card-horizontal">
                          ...
                          <div class="card-body p-5">
                            <h5 className="ubutu color-blue">{post.author_name}</h5>
                            <Typography className="ubutu gray">{post.department}</Typography>
                            <div className="d-flex flex-wrap ">
                              <div className="border-blue p-2 mt-4 text-center me-1">
                                <span className="color-blue ubutu">
                                  <b>Research Articles: </b>
                                </span>
                                <span className="color-blue ubutu">{post.researchArticlesCount}</span>
                              </div>
                              <div className="border-blue p-2 mt-4 text-center me-1">
                                <div className="text-center">
                                  <span className="color-blue ubutu">
                                    <b>h-index: </b>
                                    <span className="color-blue ubutu">44</span>
                                  </span>
                                </div>
                              </div>
                            </div>
